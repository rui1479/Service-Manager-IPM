  ...
  methods: {
    fetchData() {
      ...
      axios.get('http://localhost:3000/vehicle-types') // Get all vehicle types
        .then(response => {
          ...
          this.listServ = this.findGasoleo(this.vehicleTypes).serviços;
    ...
    findGasoleo(array) {
      for (let i = 0; i < array.length; i++) {
        if (array[i].id === 'gasoleo') {
          return array[i];
        }
      }
      return null; // Retorna null se não encontrar nenhum objeto com id 'gasoleo'
    },
    findServiceById(id) {
...
<template>
    <NavBar />
    <div id="mainpage">
        <h1 id="tipo">Adicionar serviço de combustão (gasoleo)</h1>
