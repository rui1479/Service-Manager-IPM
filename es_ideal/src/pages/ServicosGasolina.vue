    ...
    fetchData() {
      ...
      axios.get('http://localhost:3000/vehicle-types') // Get all vehicle types
        .then(response => {
          ...
          this.listServ = this.findGasolina(this.vehicleTypes).serviços;
  ...
  submitOp() {
    ...
    findGasolina(array) {
      for (let i = 0; i < array.length; i++) {
        if (array[i].id === 'gasolina') {
          return array[i];
        }
      }
      return null; // Retorna null se não encontrar nenhum objeto com id 'gasolina'
    },
    findServiceById(id) {
...
<template>
    <NavBar />
    <div id="mainpage">
        <h1 id="tipo">Adicionar serviço de combustão (gasolina)</h1>
