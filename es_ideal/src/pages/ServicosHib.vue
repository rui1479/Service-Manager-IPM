  ...
  methods: {
    fetchData() {
      ...
      axios.get('http://localhost:3000/vehicle-types') // Get all vehicle types
        .then(response => {
          ...
          this.listServ = this.findHib(this.vehicleTypes).serviços;
    ...
    findHib(array) {
      for (let i = 0; i < array.length; i++) {
        if (array[i].id === 'hibrido') {
          return array[i];
        }
      }
      return null; // Retorna null se não encontrar nenhum objeto com id 'hibrido'
    },
    findServiceById(id) {
...
<template>
    <NavBar />
    <div id="mainpage">
        <h1 id="tipo">Adicionar serviço híbrido</h1>
